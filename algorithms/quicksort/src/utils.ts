...
export const SafeArray = <T>(init: T[] = []) => {
      ...
      if (typeof prop === "string") {
        ...
        if (!isNaN(i)) {
          if (i < -target.length || i >= target.length) {
            throw new Error(`Out of bounds access on array of size ${target.length} at index ${i}`);
          }
          return target[i < 0 ? target.length + i : i];
        }
      }
      // were accessing a property, such as "sort" or "length"
